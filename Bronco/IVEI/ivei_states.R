# Description: IVEI for all Mexico (state level)
...
write.csv(estados, file = "out/votes_states.csv", row.names = FALSE)
...
estados.final <- data.frame(
    CODIGO_ESTADO = 1:32, 
    NOMBRE_ESTADO = unique(estados.porcentaje$NOMBRE_ESTADO),
    temp,
    IVEI = IVEI(temp))

write.csv(estados.final, "out/ivei_states.csv", row.names = FALSE)